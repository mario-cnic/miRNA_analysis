...
Normalización de datos de contaje crudos.
```{r}
head(readcounts)
...
Comparación 1 a 1 de todas las muestras y guardar como .jpg
```{r message=FALSE, warning=FALSE}
...
Comparación de medias de ATTR y DCM. Calcula la media de expresión para cada miRNA en cada condición y estos valores los representa en un scatter plot.
```{r}
...
Usando datos normalizados por DESeq.
```{r}
...
library(pheatmap)
```
Selecciona los miRNA significativos.
```{r}
...
head(TPM)
```
Diferentes opciones de transformación de los datos para generar el heatmap:
```{r}
#OPCION 1
...
```{r}
heatmap_matrix <- heatmap_matrix[is.finite(rowSums(heatmap_matrix)),] #Eliminar valores infinitos

colores <- colorRampPalette(c("blue", "white", "red"))(256)# Especificar paleta de color
...
## PCA 
Análisis de componentes principales (10), clusterizado por k-means y representación.
```{r}
clusterizado <- function (data) {

  #PCA + classification
pca_data <- t(data)
pca <- prcomp(pca_data) #Calculo los componentes principales de la tabla

pca_dataframe <- pca$x[,c(1:10)] # La x son los valores de las nuevas variables (o componentes principales) que ha calculado.
