...
mean_ATTR <- rowMeans(selected_columns)

selected_DCM <- trans_norm[, c(7:12)]
mean_DCM <- rowMeans(selected_columns)

mean_data <- data.frame(mean_ATTR,mean_DCM)
print(mean_data)
```
```{r}
# x<- mean_data$mean_ATTR
# y<- mean_data$mean_DCM
# model <- lm(y ~ x, data = mean_data)
...
h<-pheatmap(heatmap_matrix, cluster_row= TRUE,cluster_cols = FALSE, col=colores)
